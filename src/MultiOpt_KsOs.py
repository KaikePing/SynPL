...
# Filter data by tokenizer into train and test
flag = df['Keya'].map(lambda x: len(tokenizer(x)['input_ids'])==3)
df_pick = df[flag]

def f(x):
  var = ['Opt1', 'Opt2', 'Opt3', 'Opt4']
  if len(tokenizer(x[var[x['Ans']]])['input_ids'])!=3:
    return False
  return True

flag = df_pick.apply(f, axis=1)
df_pick = df_pick[flag]

random.seed(123)
flag = random.choice(df_pick.index, replace=False, size=137)

test = df.iloc[flag,:]
train = df.drop(flag, axis=0)
...
def Tokenize(df):
  ...
  tokenized = tokenizer(first2, third2, truncation=True, add_special_tokens=True)
